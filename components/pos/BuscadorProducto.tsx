 ...
 * Un precio de 0 significa "no se vende en esa presentación", así que se ocultan.
 * `unidadVentaId` null = unidad base suelta.
 */
function presentacionesVendibles(p: ProductoResumen) {
  const lista: {
  ...
  if (base > 0) {
    lista.push({ unidadVentaId: null, nombre: p.unidadBase, precio: base, equivale: 1 });
  }
  for (const u of p.unidadesVenta) {
    const precio = parseFloat(u.precio);
    if (precio > 0) {
      lista.push({ unidadVentaId: u.id, nombre: u.nombre, precio, equivale: u.equivale });
    }
  }
  return lista;
}
...
export function BuscadorProducto({
  ...
  /** Si se vende en varias presentaciones, pregunta cuál. Si es una sola, la agrega directo. */
  function elegirProducto(p: ProductoResumen) {
    const vendibles = presentacionesVendibles(p);
    if (vendibles.length <= 1) {
      seleccionar(p.id, vendibles[0]?.unidadVentaId ?? null);
    } else {
          ...
          {resultados.length === 0 ? (
            ...
            <ul className="divide-y divide-gray-50">
              {resultados.map(({ p, porMalestar }) => {
                ...
                const vendibles = presentacionesVendibles(p);
                const precioDesde = vendibles.length
                  ? Math.min(...vendibles.map((v) => v.precio))
                ...
                return (
                  <li key={p.id}>
                      ...
                      <div className="text-right shrink-0 flex items-center gap-1.5">
                        ...
                        <div>
                          <p className="font-bold text-[#1e3a5f] text-sm">
                            {vendibles.length > 1 && (
                              <span className="text-[10px] font-normal text-gray-400">desde </span>
                            )}
                            Q {precioDesde.toFixed(2)}
                          </p>
                    ...
                    {/* Presentaciones vendibles: solo se muestra si hay más de una */}
                    {abiertoEste && (
                      <div className="bg-gray-50/80 border-t border-gray-100 px-3 py-2">
                        ...
                        <div className="space-y-1">
                          {vendibles.map((v) => {
                            const alcanza = p.stock >= v.equivale;
                            const esBase = v.unidadVentaId === null;
                            return (
                              <button
                                key={v.unidadVentaId ?? "base"}
                                type="button"
                                disabled={!alcanza}
                                onClick={() => seleccionar(p.id, v.unidadVentaId)}
                                className={cn(
                              ...
                              >
                                <span className="text-gray-700">
                                  ...
                                  <span className="text-gray-400 text-xs">
                                    {esBase ? " · suelta" : ` · ${v.equivale} ${base}s`}
                                  </span>
                                  {!alcanza && (
            ...
            <div className="space-y-4 py-1">
              ...
              <div className="border-t border-gray-100 pt-3">
                ...
                <div className="space-y-1 text-sm">
                  {/* La unidad suelta solo si de verdad se vende suelta (precio > 0) */}
                  {parseFloat(detalle.precioVenta) > 0 && (
                  ...
                  {[...detalle.unidadesVenta]
                    .filter((u) => parseFloat(u.precio) > 0)
